    ...
    def returnPwdDic(self):
        ...
        if algo == '1.2.840.113549.1.12.5.1.3' or algo == '1.2.840.113549.1.5.13':
            for i in logins:
                ...
                loginList[i][0] = ('%20s:' % (i[2]))

                # username
                iv = i[0][1]
                ciphertext = i[0][2]
                print(unpad(DES3.new(key, DES3.MODE_CBC, iv).decrypt(ciphertext), 8), end=',')
                loginList[i][1] = (unpad(DES3.new(key, DES3.MODE_CBC, iv).decrypt(ciphertext), 8))

                # password
                iv = i[1][1]
                ciphertext = i[1][2]
                print(unpad(DES3.new(key, DES3.MODE_CBC, iv).decrypt(ciphertext), 8))
                loginList[i][2] = (unpad(DES3.new(key, DES3.MODE_CBC, iv).decrypt(ciphertext), 8))

        return logins

    '''
    # main functions down here
